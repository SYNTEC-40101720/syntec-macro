...
本矩阵把控制器能力、版本基线、插件行为和验证证据放在同一张表中；“已登记”不等于“已在所有控制器/机型实测”。历史迭代与变更记录见根目录 `CHANGELOG.md`。

更新日期：2026-09-22
状态：能力登记与首轮审计已完成；Phase 5.1 控制器实测复核已于 2026-09-22 在 Syntec 81RA / 11MA（10.120.44C / 10.120.52）完成 B 级采集，CALL-RUN-01..07 / FUN-A-13A..F / ROB-LTP-01..08 实测结论已回填至下表对应行；CALL-001/002、FUN-001、ROB-001 状态从 «部分核实» 升级为 «实测复核»，Rust Phase 5.2-5.3 跨行 emit 已解锁。
...
| CALL-001 | `G65/G66/G66.1/G67` | CF 已确认 G66/G66.1 基础触发语义和最后 G 码顺序；变量空间、模式取消及产品差异仍分开记录。Phase 5.1 CF 范围复核已确认 81RA 与 CF 规范差异为机型分支特性（CNC 以 CF 规范为准）。Phase 5.1 控制器实测已确认 CALL-RUN-03/04 在 CNC（81RA / 11MA, 10.120.44C / 10.120.52）与 CF 规范一致，81RA 旧版"只触发一次"差异已被 10.120.44C+ 机器人版本修正。 | 静态定义/引用解析，G/M hover，非最后 G 码 warning。 | 实测复核 | 维持 CF 每移动/每单节触发模型为默认；81RA 旧版分支不再单独适配。 |
| CALL-002 | `M98/M198/M99` | `P/H/L`、返回、变量继承、`M198` 重读已整理。Phase 5.1 CF 范围复核已确认 CALL-RUN-01/02/05/06/07 与 CF 规范一致（通过）；控制器实测复核已确认 CALL-RUN-01/02/05/06/07A/B/C 全部通过（B 级证据）。 | 静态定义/引用解析，G/M hover。 | 实测复核 | M99 返回与变量继承边界已随 CALL-RUN 系列控制器实测复核；后续变更需重新采集。 |
...
| FUN-001 | 62 个内置函数 | 函数表是正式来源，已完成多类别首轮审计，FUN-A-13 B 级实测已完成（Phase 5.1 控制器实测复核）。`GETPR/SETPR` 签名多态重载、返回类型 `float (Double)`、`0.0` fallback、越界 `COR-016` / 超范围 `COR-023` / 权限 `COR-024` 警报与即时/重启二分生效机制均已收口。 | `src/functions.js` 定义 62 个函数，补全和 hover 已覆盖。 | 实测复核（FUN-A-13 已解锁） | Phase 5.2-5.3 Rust 端可针对 GETPR/SETPR 接入 emit 入口；继续按类别核对其他函数签名与版本。 |
| FUN-002 | `SYSDATA/DRVDATA` | `SYSDATA` 需整数诊断号与 `WAIT()`；`DRVDATA` 对版本、站号和启动时间敏感。首轮审计多类别已核实。 | 参数诊断、hover、补全。 | 已核实 | 后续按函数批次继续核对版本差异。 |
| FUN-003 | 系统控制函数 | `ALARM/MSG` ID 边界、`WAIT` 预解例外和 `CHK*` 返回值已核实。 | 参数诊断、hover、补全。 | 已核实 | 运行时行为仅按 hover 说明，不做静态判断。 |
| ROB-001 | LTP 机器人专项语法 | LTP 正式《语法指令规格》与《机器人语法对应 G 码与支援版本》及 20 个独立页面已通过 Confluence 复核；静态范围、Q 联动和坐标系禁用写法已取得可实现证据。Phase 5.1 控制器实测复核已确认 ROB-LTP-01..08 实测警报编号（`RBT-124/127/115/322/154-2/110/257/123`）与 CF 文档级基线对齐。Rust `RobotLineState` 现有字段已覆盖 Phase 5.1 实测所需跟踪的跨行状态。对应 23 个 `SYNTEC_ROBOT_*` 稳定诊断 code（详见 [Rust诊断parity清单](Rust诊断parity清单.md)「诊断 code ↔ 能力矩阵交叉引用」段）；原 3 个 dead code（`SYNTEC_ROBOT_SWAITSIG_Q_RANGE` / `SYNTEC_ROBOT_SYNCOUT_Q_RANGE` / `SYNTEC_ROBOT_SKIPCOND_Q_RANGE`）于 2026-09-21 两端共同剔除，总 code 67→64；原有诊断行为不变（`SYNTEC_ROBOT_STATIC_ARG_RANGE` 覆盖 SKIPCOND/SWAITSIG/SYNCOUT 三 command 的 Q range 警报）。Phase 5.1 后若需按实测 RBT 编号新增独立 code，仍按四件套登记重新落回。 | Rust `validate_robot_line_state` 跨行状态机 + 静态范围诊断；已覆盖 `USERCOR/TOOLCOR/G68.18/POSEMAP/SHIFTON/SKIPCOND/SWAITSIG/SYNCOUT/CIRMODE/G192.1/WAITSYNC` 等静态范围；跨行状态 `MOVC pair` / SWAITSIG/SYNCOUT counters / STITCHON/WEAVEON/WAITSYNC/G192.1 生效范围禁忌通过 4 个区间禁忌独立 code 处理。 | 实测复核（CF + B 级双轨对齐） | `RBT-118` 点位偏移侧编号仍待后续专项采集；其余 RBT 编号均已收口 Rust emit。 |
...
| FUN-E | 数学、字符串、单位与堆栈 | P2，首轮已核实 ABS/SIN/COS/TAN/ATAN/ATAN2/EXP/LN/POW/ACOS/ASIN/STR2INT/SCANTEXT/SQRT/CEIL/FLOOR/ROUND/STD/STDAX/MAX/MIN/SIGN/RANDOM/堆栈资料 | 已修正 ATAN2 象限样例与定义域说明；`LN/POW/ACOS/ASIN/SQRT` 常量定义域、基础数学函数、`CEIL/FLOOR/ROUND`、`STD/STDAX`、`MAX/MIN/SIGN/RANDOM` 与堆栈函数已完成首轮审计并建立回归，其余函数继续分批核实型别和版本。 |
| FUN-F | 图形模拟：`SETDRAW`、`DRAWHOLE` | P2，首轮已核实 BGR 色码、状态恢复和圆形绘制语义 | hover 回归已就位；颜色、半径与实际模拟渲染仍待目标模拟器验证。 |
