...
### 4.1.5 区间未关闭收尾诊断复核（2026-09-23）

针对「STITCHON/WEAVEON/WAITSYNC/G192 区间未关闭（忘记写 STITCHOFF/WEAVEOFF/ENDSYNC/G192.2）是否应作为静态语法诊断 emit」的复核，结论为**不实现**：

- **STITCHON/STITCHOFF**（规格页 64815646）：明确「RESET/加工结束视为自动执行 STITCHOFF」。文件尾/M30/RESET 即自动关闭区间，非错误状态，不应 emit。
- **WEAVEON/WEAVEOFF**（64815586）、**WAITSYNC/ENDSYNC**（64815317）、**G192.1/G192.2**（64817515）：三页仅登记区间内禁用指令触发的 `RBT-322`/`RBT-257`/`RBT-123`，以及运行时警报（如 WAITSYNC §注意事项 6「追踪物体超工作范围自动解除并发警报」）。**无「忘记关闭区间」的静态语法诊断记载**。
- 控制器对未关闭区间要么自动关闭（STITCHON），要么属运行时行为，静态诊断无规范依据。新增无证据诊断违反「诊断规则科学化工作流」第 1 步「规范手册对齐」。
- 若后续控制器版本补充区间未关闭的静态警报编号（如新的 RBT-XXX），再按四件套登记补 emit 入口。
