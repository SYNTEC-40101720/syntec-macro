...
| 2026-09-17 | v2.15.0 发布 | 详见 CHANGELOG v2.15.0 段。 |
| 2026-09-19 | M3 Rust/Wasm CLI 试点 + WASM ABI + adapter | Rust CLI/Wasm ABI 子集实现；RustWasmAdapter 进入开发态差分链路，不进入生产 Provider。 |
| 2026-09-20 | M3 / P0-A 稳定诊断 parity 67/67 收口 | TOOLCOR/ROBOT-MOV/MODBUS/STITCH-WEAVE/SIGNAL 5 批机器人规则组全 parity；GOTO/G65/G66.1/M98/M198/M99/G67 调用返回 + % 缺 %@MACRO 文件头 warning parity。 |
| 2026-09-20 | M3 / P0-B 真实 Request + 完整 AnalysisResult parity | Rust/Wasm 130+10+17 等价（diagnostics / navigation / edits）。 |
| 2026-09-20 | M3 / P0-C 生产 Wasm 资产 + Worker 注入 + Lifecycle | ustWasmAsset.js/ustWasmWorkerAdapter.js + workerLifecycle.test.js 9 项集成测试。 |
| 2026-09-20 | M3 / P1 JS/Rust/Wasm 三方对照基准 + 阈值 + CI 跨平台矩阵 + readiness 自检 + 3.x Runbook | enchmarkCompare.js+comparePerfData.js+checkReleaseReadiness.js，.github/workflows/rust-wasm.yml 升级 ubuntu/windows 矩阵 + perf-data artifact + cross-platform alert。 |
| 2026-09-20 | **v3.0.0 GitHub Release 发布** | 版本号 Major 升级到 3.0.0；.vscodeignore 把 ssets/rust-wasm/ bundle 进 VSIX；check:vsix 44 文件；默认 backend 仍是 javascript（实测 Rust 在 large-20k 仍存微弱回归）；Release URL: https://github.com/SYNTEC-40101720/syntec-macro/releases/tag/v3.0.0 |
