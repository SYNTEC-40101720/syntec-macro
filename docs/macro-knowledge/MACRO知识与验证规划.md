...
`v2.10.0`、`v2.11.0` 与 `v2.11.5` 均已正式发布。本规划承接后续 MACRO 知识验证和能力实施，新增功能应进入后续版本，避免回写已发布版本的收口范围。
...
当前状态：G0/G1 已完成，`v2.10.0` 与 `v2.11.0` tag 均已推送并发布；CALL-01 至 CALL-03 的资料收集已完成，详见 [MACRO 调用语义资料包](MACRO调用语义资料包.md)；能力矩阵已建立，详见 [MACRO 能力矩阵](MACRO能力矩阵.md)；FUN-A 至 FUN-F 已完成首轮审计与保守实现，G2 继续补齐逐机型与运行时证据。
...
状态：静态导航、Hover 与资料收集已完成首轮实施；CALL-RUN 运行时验证持续进行。
