...
`v2.10.0` 至 `v2.13.0` 均已正式发布。本规划承接后续 MACRO 知识验证和能力实施，新增功能应进入后续版本，避免回写已发布版本的收口范围。
...
当前状态：G0/G1 已完成，`v2.10.0` 至 `v2.13.0` 已推送并发布；CALL-01 至 CALL-13 的资料、静态导航边界和首轮自动化验证已完成，CF TechManual 的 C-Type 与当前 `G66.1` 页面已确认 CNC 侧的 `G66/G66.1` 触发语义、完整范例和最后 G 码限制；页面未提供最低软件版本或 81RA 例外说明，因此 81RA 与 CF 不一致的运行时行为仍待 CNC 模拟器/控制器复核；ROB-001 已取得 LTP《语法指令规格》和版本矩阵，主要机器人指令、替代 G 码和部分最低版本已登记，完整参数/警报/机型/运行时条件仍待逐项验证；能力矩阵已建立，详见 [MACRO 能力矩阵](MACRO能力矩阵.md)；FUN-A 至 FUN-F 已完成首轮审计与保守实现；`v2.12.0` 的 R 保留区扩展和控制流嵌套深度 warning，以及 `v2.13.0` 的共享词法、LTP 静态范围诊断和导航并发改进，均已完成代码、文档与回归同步，后续再按证据成熟度推进 G2。
