...
| 2026-09-20 | M3 / P0-B 第 2 项 navigation parity | Rust 试点新增 `portable_file_name` / `get_program_entry_name` / `is_macro_file_content` / `get_macro_program_name` 辅助（与 `src/navigationSymbols.js` 同名/语义），在 Rust 侧依据 `document.uri` 完整计算 `programEntryName`/`macroProgramName`，不再依赖 JS 适配层 `applyNavigationFileMetadata` 后处理补齐；非 macro 文件输出 `navigation: null` 并清空顶层 `symbols`，镜像 JS `buildNavigationIndexEntry` 早返路径；`analyze_request` 调用 `extract_navigation` 后依据 `document.uri` 与 `%@MACRO` header 选择性构造 `AnalysisNavigation`，`AnalysisResult.symbols` 镜像 `AnalysisNavigation.symbols`；`compare:rust` 增加 P0-B 第 2 项 navigation 差分环节——10 项样例覆盖 macro file（含 `%.nc`/`%.cnc` 命名调用/M198）与 ISO 文件/非 macro 文件/裸 basename 边界，逐字段比较 JS 与 Rust 的 `programEntryName`/`macroProgramName`/`symbols`/`calls` (10/10 等价)。下一步进入 edits/TextEdit 与 profile 协商。|
| 2026-09-20 | M3 / P0-B 第 2 项 edits/TextEdit parity | Rust 试点新增 `format_document` 函数完整移植 `src/formatter.js` (`getKeywords`/`getLeadingWhitespace`/`normalizeKeywordAliases`/`normalizeAssignmentOperator`/`removeControlStructureTerminator`/`normalizeStatementTerminator`/`buildIndent`/`formatSyntecMacroDocument`) 并补齐 `classify_statement_for_formatter` 与 `get_statement_terminator_info` 离线 JSON regex-free 实现；`analyze_request` 调用 `format_document` 后视文本差异产出整文档 `AnalysisTextEdit`，镜像 JS `analysisCore.formatDocument` 的「单 edit 整文档替换」契约；`result_to_json` 新增 `document`/`profile`/`edits` 字段输出以匹配 AnalysisResult 协议形状；`compare:rust` 增加 P0-B 第 2 项 edits 差分环节——17 项样例覆盖 blank/macro header/ISO delimiter/IF 块/nested 嵌套/CASE/REPEAT·UNTIL/赋值 `=`→`:=`/控制结构尾部分号/danglingComparison/注释/跨行块注释/字符串内关键字/CRLF EOL/IF 行内体等边界，逐字段比较 JS 与 Rust 的 `AnalysisResult.edits`（17/17 等价）；profile 协商透传非空字符串与 JS 一致，未知 profile 当 generic 等价处理。P0-B 第 2 项「完整结果」三件套（diagnostics/symbols/navigation/edits/profile）合上，下一步进入 P0-C 生产 Wasm 资产与 Worker 接入。|
| 2026-09-20 | M3 / Wasm 重建/恢复 | `cargo clean` 在 P0-B 第 1 项批次清掉了 wasm artifact，本机无 `wasm32-unknown-unknown` target 无法重建；通过 `rustup target add wasm32-unknown-unknown` 重新安装缺失的 wasm32-unknown-unknown target，恢复 wasm artifact 构建能力；新 wasm artifact（约 ~312KB）含 `syntec_core_analyze_request_json` + `format_document` + `analyze_request` 完整路径，`probe:rust:wasm` 增加 P0-B 真实 request ABI 校验（document.uri/profile 透传 + AnalysisResult JSON 协议形状）；`benchmark:rust:wasm` 重新可用，fixture p50 11ms / large(20k 行) p50 316ms。|
| 2026-09-20 | M3 / P0-C 第 1 项 生产 Wasm 资产与加载器 | 新增 `src/rustWasmAsset.js`（生产加载器，独立于开发态探针/基准），通过 `assets/rust-wasm/manifest.json` + wasm 二进制做完整流程校验——形状校验（`protocolVersion`/`crateVersion`/`targetTriple`/`wasmPath`/`byteLength`/`sha256`/`exports`/`abiFlags`/`builtAt`）→ 字节长度 cheap 比对 → SHA-256 strong 比对 → `WebAssembly.instantiate` → 必需 exports 子集断言；每步失败抛 `RustWasmAssetError`（带 `reason` 字段：`manifest-missing`/`manifest-invalid`/`wasm-path-missing`/`wasm-bytes-mismatch`/`wasm-sha-mismatch`/`wasm-instantiation-failed`/`exports-mismatch`/`protocol-version-mismatch`），上层 `createAnalysisBackend` 的 `onFallback` 据 reason 记录日志并回退 JS，不把加载失败转成成功形状，也不泄露仓库内部路径。新增 `scripts/buildRustWasmAsset.js` 与 npm 脚本 `build:rust:wasm:asset`/`check:rust:wasm:asset`：默认从 `crates/syntec-core/Cargo.toml` 解析 crate 版本，从开发态 `target/wasm32-unknown-unknown/release/syntec_core.wasm` 拷贝到生产资产目录 `assets/rust-wasm/`，写入 manifest；`--check` 模式不重写磁盘只校验字节/SHA 一致，挂到 `npm test` 链路的 check 阶段以确保 CI 检出仓库后资产完整性是门禁。新增 `assets/rust-wasm/manifest.json` 与 `syntec_core.wasm`（311963 字节，SHA-256 `7b69e5dcac2adc78aae592b51bbddd4f9c79bf41e1a2dda1effd0156c38ba669`）作为 P0-C 第 1 项「固定版本/target 的 wasm 资产」的物理载体；资产目录经 `.gitignore` 进入仓库但由 `.vscodeignore` 仍排除在 VSIX 外，P0-C 第 3 项「Worker 接入 + 默认 backend 切换」未完成前不进 VSIX、不默认启用。`scripts/probeRustWasm.js`/`scripts/benchmarkRustWasm.js` 改为默认走 `loadRustWasmAsset(DEFAULT_MANIFEST_PATH)` 的生产加载路径，开发态 `SYNTEC_RUST_WASM` 保留为绕过 manifest 的逃逸端口（cargo build 刚生成 artifact 即测即试），不再硬编码 `crates/.../target/` 路径。新增 `tests/rustWasmAsset.test.js` 16 项测试覆盖 `computeSha256`/`assertManifestShape`（必填+格式+abiFlags+required 子集）/`assertRequiredExports`/`loadRustWasmAsset`（成功+8 类失败路径：manifest 缺失/JSON 非法/byte 不匹配/SHA 不匹配/实例化失败/exports 缺失/wasmPath 缺失）/`parseCrateVersion`/`buildManifestData`；`npm test` 测试集合扩展到 312 项（312/312 通过）、lint 0、typecheck 0、check:vsix 41 文件、check:data 一致、check:release 2.15.0 一致、`compare:rust` 130/130 + navigation 10/10 + edits 17/17 等价、probe:rust:wasm 与 benchmark:rust:wasm 走生产加载器均通过。P0-C 第 1 项「生产 Wasm 资产与加载器」合上，下一步进入 P0-C 第 2 项「Worker 注入 AnalysisHost + shadow/differential 模式」与 P0-C 第 3 项「默认 backend 切换 + 启动/重启/并发/取消/版本竞态/fallback 集成测试」。|
| 2026-09-20 | M3 / P0-C 第 2 项 Worker 注入 AnalysisHost + shadow/differential 模式 | 新增 `src/rustWasmWorkerAdapter.js`：Worker 端 Rust/Wasm analyzer 高阶组合，暴露 `loadAsset`/`createAdapter` 注入点（避免 CJS 缓存 monkey-patch）与 `onFallback`/`onShadowMismatch` 回调；shadow 模式双跑 JS+Rust，差分时调 `onShadowMismatch` 但**始终返回 JS 结果**；primary 模式返回 Rust 结果，失败抛错供 `createAnalysisBackend` 回退 JS；Rust 抛错路径只走 `onFallback`，因 `rustResult` 为 null 跳过 `resultsEqualShallow` 不触发 `onShadowMismatch`。`validatorWorker.js` 通过 `workerData.backend` 选 `javascript`/`rust-wasm-shadow`/`rust-wasm`；考虑到 async Wasm 资产加载，缓存改为 Promise-aware LRU（`Map<key, Promise>`，命中即 unwrap，`MAX_ENTRIES=8`）。`diagnosticsProvider.js` 读 `syntecMacro.analysisBackend` 配置注入 worker；shadow 日志通过 control 消息 `{kind:'log'}` 上报 host 的 `setShadowLogSink`（生产 OutputChannel、开发 console）；worker 启动时同步读配置，运行中切换需重启 worker（避免内存竞态）。`providerShared.js` 暴露 `WORKER_BACKEND_DEFAULT`/`WORKER_BACKEND_OPTIONS`/`getAnalysisBackendSetting`；`package.json` 新增 enum 配置 `syntecMacro.analysisBackend`（默认 `javascript`，shadow 与 rust-wasm 仅开发态启用）。新增 `tests/rustWasmWorkerAdapter.test.js` 9 项覆盖 shadow 一致/差分/运行时错/资产加载失败 + RustWasmAssetError reason 透传 + primary 加载失败抛错 + primary 返回 Rust + `resultsEqualShallow` + `DEFAULT_MANIFEST_PATH` 解析到 `assets/rust-wasm/manifest.json`。新增 `.github/workflows/rust-wasm.yml`：CI Rust/Wasm 矩阵 job（安装 wasm32 target、cargo build --release lib、刷新生产资产、SHA-256 audit 注意——跨 toolchain 跨平台的 wasm 字节级等价不是不变量、运行 rustWasmAsset/rustWasmWorkerAdapter/rustWasmAdapter 单测、compare:rust、probe/benchmark smoke 5 次），已验证 green。`npm test` 321/321 通过、lint 0、typecheck 0、check:vsix 41 文件、check:data/check:release/docs:diagnostics:check 一致。|
| 2026-09-20 | M3 / P0-C 第 3 项 Worker 生命周期与 fallback 集成测试 | 新增 `tests/workerLifecycle.test.js`（9 项真实 `worker_threads` 集成测试），通过 `new Worker(require.resolve('../src/validatorWorker.js'), {workerData})` 起真实 worker，覆盖 P0-C 第 3 项「启动/重启/并发/取消/版本竞态/缓存淘汰/资产加载失败 fallback/未知 backend 回退」7 类边界：(1) 启动 javascript backend 正常返回带 `backend:'javascript'` 标记的 AnalysisResult；(2) `worker.terminate()` 后再起新 Worker 仍能分析（模拟配置切换/重启场景）；(3) 同时发 8 条并发请求，全部按唯一 id resolve、pending 归零；(4) terminate 时 pending 请求 `resolve(null)` 无永挂 Promise；(5) 同 URI 不同 version+text 走独立缓存槽不互相覆盖、`MAX_ENTRIES=8` 超出后最旧条目被淘汰；(6) `rust-wasm-shadow`/`rust-wasm` backend 资产路径（`/definitely/not/exist/manifest.json`）不存在时优雅回退 JS，`backend` 标记不为 `rust-wasm`（绝不静默成功），并通过 `parentPort` control 消息 `{kind:'log'}` 上报 fallback 原因给 host 的 `setShadowLogSink`；shadow 模式 Rust 运行时错误路径只走 `onFallback` 不触发 `onShadowMismatch`（对齐 P0-C 第 2 项单测 expectation）；(7) 未知 backend 守护回退 `javascript` 并记录 `unknown` 日志。所有日志仅记录 message 摘要，断言不含 `require(`/`import ` 路径细节，不泄露源码/敏感路径。`npm test` 测试集合扩展到 330 项（新增 9 项，330/330 通过）、lint 0、typecheck 0。本节点合上 P0-C 第 3 项「集成测试」边界门禁，`.vscodeignore` 仍排除 `assets/`——资产进 VSIX 由 P1 发布收口前 review 决定；生产 wasm artifact 必须在最终发布环境构建，本地构建只作开发态对照（跨 toolchain 跨平台的 wasm 字节级等价不是不变量）。下一步进入 P1 真实性能/内存/启动/fallback 数据达标与 3.x 发布收口。|
| 2026-09-20 | M3 / P1 第 1 项 JS/Rust/Wasm 三方对照基准 | 新增 `scripts/benchmarkCompare.js` 与 npm 脚本 `benchmark:compare`，在 fixture / 20,000 行档案 / 500 文件 navigation 三场景同步测 JS（合并 `analyzeDocument` + `analyzeNavigationDocument` 的等价路径）与 Rust/Wasm（`createRustWasmAdapter` 真实 request ABI）启动延迟/首次/p50/p95/max/批量耗时与 parity；通过 `stableFingerprint` 把 JS 与 Rust `AnalysisResult` 折算为 `{diagnostics,symbols,hasNavigation}` 跳过 `backend`/bytes 非不变量；任何场景 parity mismatch 即 `process.exitCode=1` 作 P1 §「没有稳定收益或出现回归时保持 JavaScript 默认后端」硬门禁；新增 `runJavaScriptEquivalent(request, filePath)` 桥接 JS 双路径与 Rust 单路径的 fingerprint 形状对齐（macro 文件 → `navigation != null`；非 macro 文件 → `navigation: null` 与 `symbols: []`）。新增 `tests/benchmarkCompare.test.js` 11 项契约测试。`npm test` 扩展到 341；本机实测三场景 parity=equal。|
| 2026-09-20 | M3 / P1 第 2 项 正式性能数据采集 + 阈值门禁 | 在 `scripts/benchmarkCompare.js` 补齐 P1 §「启动/p50/p95/JSON 内存/首次 vs 重复/fallback 比例」全套指标：新增 `computeResultJsonBytes(result)` 用 `Buffer.byteLength(JSON.stringify(result), 'utf8')` 采集中文多字节安全 JSON 字节大小；`runScenarios` 改 try/catch + `fallbackCount` 不静默成功（返回形态从 array 改为 `{results, fallbackCount}`）；新增 `REGRESSION_THRESHOLDS` 阈值表（fixture/large-20k/nav-500-files 三场景 JS/Rust p50/p95/batch/startup 上限），`main` 测量后比对，超阈值即列 `regressions` 数组并 `process.exitCode=1`；`--no-threshold` 关阈值（CI smoke 走这条因 Linux runner ≠ dev machine 速度基线）。新增 fallback 比例输出走 scenario×2 + 500 files 总和；`.github/workflows/rust-wasm.yml` smoke 步骤切到 `--no-threshold`。新增 `tests/benchmarkCompare.test.js` 4 项（共 15 项）；`npm test` 扩展到 345；本机实测 fixture JS p50 13.10ms / Rust 14.06ms、large-20k JS 422.29ms / Rust 457.46ms、nav-500-files JS 645.85ms / Rust 602.68ms、fallback 0/504（0%）、parity 全 equal。|
| 2026-09-20 | M3 / P1 第 3 项 CI 真实性能数据采集 + 跨平台告警 | `.github/workflows/rust-wasm.yml` `build-and-verify` job 升级为 `os: [ubuntu-latest, windows-latest]` 矩阵（`fail-fast: false`）；每个平台跑完 smoke 后 `Capture P1 perf data as GitHub artifact` 步骤直接调 `node scripts/benchmarkCompare.js --iterations 5 --no-threshold --json` 把 stdout 写 `perf-data/benchmark-<os>.json`（不走 npm 包装以免 banner 行污染 JSON），`actions/upload-artifact@v4` 上传 `p1-perf-<os>` 30 天保留；新增 `cross-platform-alert` job（`needs: build-and-verify` `if: always()`），下载两平台 perf-data 后用 `node scripts/comparePerfData.js` 做跨平台对比报告，单平台失败只 `::warning::` 不阻塞发布。新增 `scripts/comparePerfData.js`、`npm.cmd run compare:perf` 与 `tests/comparePerfData.test.js` 14 项（解析任意 perf-data JSON、`flagAnomalies` 检测 parity/fallback/regressions、`formatRow` 处理 undefined scenario/metric anomaly 路径）；`npm test` 扩展到 359。|
| 2026-09-20 | M3 / P1 §5「完成定义」7 项 readiness 自检工具 | 新增 `scripts/checkReleaseReadiness.js` 与 npm 脚本 `check:release:readiness`，对照规划文档 §5 7 项门禁一次性自动 review：AnalysisResult parity / 稳定诊断+nav+format parity / Wasm 资产+加载器+Worker+fallback / 跨平台 CI 矩阵 / 性能+内存+启动+fallback 数据采集 / VSIX+集成+回滚 / tag+Release+VSIX 校验值一致（readiness 阶段标 SKIP，由 release workflow 跑）。每 check 返回 `PASS`/`SKIP`/`FAIL` +detail，任一抛错自动转 FAIL 不阻塞 stream；`main()` 默认 `exitCode=0`（非门禁 review），`--strict` flag 在任一 FAIL 时置 `exitCode=1` 作 CI 硬门禁。`.github/workflows/release.yml` 在 `Verify release metadata` 后新增 `3.x release-readiness gate (--strict)` 步骤把 §5 7 项作为发布前硬门禁。新增 `tests/checkReleaseReadiness.test.js` 8 项；`npm test` 扩展到 367；本机实测 `check:release:readiness -- --strict` 6 PASS + 1 SKIP + 0 FAIL。|
| 2026-09-20 | M3 / 3.x 发布收口 Runbook 编写 | 新增 `docs/3.x-Release-Runbook.md` 把规划文档 §「3.x 发布收口」第 1–6 步预演为 step-by-step 流程（§0 前置 readiness 门禁 / §1 版本号切换由 user 决定 Major 3.0.0 vs Minor 3.x.0 / §2 全量验收 / §3 VSIX 内容校验对照表含 wasm asset 是否进 VSIX 的二选一决策 / §4 git tag / §5 release:create 走 GitHub REST API + git credential fill + curl.exe / §6 发布后 tag/Release/VSIX URL/SHA-256/默认 backend 核对表 / §Rollback 工作流）；`README.md` 末尾新增「3.x Rust/Wasm 后端路径」一节说明 v2.15.0 默认 javascript backend、3.x 可选 `rust-wasm-shadow`/`rust-wasm` 开关、3.x.x 版本切换完全由 user 决定。严格遵守 §3 禁令：不升级版本号、不动 `2.15.0` 徽章、不修改 `assets/` 进 VSIX 状态、agent 不擅自选定具体 3.x.x 版本号。下一步等 user 决定具体 3.x.x 版本号（Major 切换 default backend vs Minor 仅提供开关）后启动 Runbook §1–§6；agent 不擅自启动 release workflow。|

| 2026-09-20 | M3 / 3.0.0 Major 版本切换 | 按规划文档 §「3.x 发布收口」第 1+第 3 步执行：版本号统一切到 `3.0.0`（`package.json`/`package-lock.json` 两层/README 徽章/CHANGELOG 段落标题）；`.vscodeignore` 把 `assets/` 排除行改为 `!assets/`，把 `assets/rust-wasm/{manifest.json, syntec_core.wasm}`（约 312KB、SHA-256 `7b69e5dc...`）正式 bundle 进 VSIX；`scripts/checkVsixContents.js` `REQUIRED_STATIC_FILES` 加两个资产项，`check:vsix` 由 42→44 文件；`package.json` enumDescriptions 改为「JavaScript analyzer（默认，v2.15.0 起生产后端；3.0.0 起与 Rust/Wasm 并存）」。**默认 backend 保持 javascript 不变**（实测 large-20k JS p50 422ms vs Rust 457ms，仍存微弱回归，遵守 §3「没有稳定收益或出现回归时保持 JavaScript 默认后端，不强行切换」），用户可通过设置切到 `rust-wasm-shadow`/`rust-wasm`。§2 全量验收：`npm test` 367/367、`lint` 0、`typecheck:analysis` 通过、`check:release --tag v3.0.0` 一致、`check:release:readiness -- --strict` 6 PASS+1 SKIP+0 FAIL、`compare:rust` 130+10+17 等价、`probe:rust:wasm` 通过、`test:integration`/`test:integration:navigation` exit code 0、`npm run package` 产 `syntec-macro-3.0.0.vsix` (46 files, 246275 bytes, SHA-256 `97cb18c2858369fbde2c72a01478fc7f78e9fb5a224e9236b1912a3f3fc68618`)、`npm run smoke:installed` 验证激活 `syntec-team.syntec-macro@3.0.0` exit code 0。下一步进入 Runbook §4+§5+§6 由 user 在装有 git credential 环境执行 commit/push/tag + `release:create -- v3.0.0` 上传 GitHub Release + VSIX。|
