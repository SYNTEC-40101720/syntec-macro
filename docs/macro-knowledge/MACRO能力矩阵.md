...
更新日期：2026-09-22
状态：G0/G1/G3 已完成；`v2.10.0` 至 `v2.14.0` 已正式发布，G2 已完成调用资料、静态导航边界、CALL-13 warning、R 保留区扩展、控制流嵌套深度 warning、共享词法状态机、LTP 静态范围/Q 联动诊断、导航文件监听缓存和 FUN-A 至 FUN-F 首轮审计；Phase 5.1 控制器实测复核已于 2026-09-22 在 Syntec 81RA / 11MA（10.120.44C / 10.120.52）完成 B 级采集（user 张颖 2026-09-20~22），CALL-RUN-01..07 / FUN-A-13A..F / ROB-LTP-01..08 全部回填至对应资料包；CALL-001/002、FUN-001、ROB-001 状态从 «部分核实» 升级为 «实测复核»，Rust Phase 5.2-5.3 解锁。
...
| CALL-001 | `G65/G66/G66.1/G67` | CF 已确认 G66/G66.1 基础触发语义和最后 G 码顺序；变量空间、模式取消及产品差异仍分开记录。Phase 5.1 CF 范围复核已确认 81RA 与 CF 规范差异为机型分支特性（CNC 以 CF 规范为准），详见 [调用语义资料包 §3.2](MACRO调用语义资料包.md#32-phase-51-cf-规范范围复核结论2026-09-20)。Phase 5.1 控制器实测已确认 CALL-RUN-03/04 在 CNC（81RA / 11MA, 10.120.44C / 10.120.52）与 CF 规范一致，81RA 旧版"只触发一次"差异已被 10.120.44C+ 机器人版本修正，详见 [调用语义资料包 §3.3 / §4.1](MACRO调用语义资料包.md#33-phase-51-控制器实测复核结论2026-09-22)。 | 静态定义/引用解析，G/M hover，非最后 G 码 warning。 | 实测复核 | 维持 CF 每移动/每单节触发模型为默认；81RA 旧版分支不再单独适配。 |
| CALL-002 | `M98/M198/M99` | `P/H/L`、返回、变量继承、`M198` 重读已整理。Phase 5.1 CF 范围复核已确认 CALL-RUN-01/02/05/06/07 与 CF 规范一致（通过），详见 [调用语义资料包 §3.2](MACRO调用语义资料包.md#32-phase-51-cf-规范范围复核结论2026-09-20)。Phase 5.1 控制器实测复核已确认 CALL-RUN-01/02/05/06/07A/B/C 全部通过（B 级证据回填），详见 [调用语义资料包 §3.3 / §4.1](MACRO调用语义资料包.md#33-phase-51-控制器实测复核结论2026-09-22)。 | 静态定义/引用解析，G/M hover。 | 实测复核 | 见 [调用语义资料包](MACRO调用语义资料包.md)。 |
...
| FUN-001 | 62 个内置函数 | 函数表是正式来源，已完成多类别首轮审计，FUN-A-13 B 级实测已完成（Phase 5.1 控制器实测复核，详见 [函数审计资料包 §4.2](MACRO函数审计资料包.md#42-phase-51-控制器实测复核结论2026-09-22)）。`GETPR/SETPR` 签名多态重载、返回类型 `float (Double)`、`0.0` fallback、越界 `COR-016` / 超范围 `COR-023` / 权限 `COR-024` 警报与即时/重启二分生效机制均已收口。 | `src/functions.js` 定义 62 个函数，补全和 hover 已覆盖。 | 实测复核（FUN-A-13 已解锁） | Phase 5.2-5.3 Rust 端可针对 GETPR/SETPR 接入 emit 入口；继续按类别核对其他函数签名与版本。 |
...
| ROB-001 | LTP 机器人专项语法 | LTP 正式《语法指令规格》与《机器人语法对应 G 码与支援版本》及 20 个独立页面已通过 Rovo MCP 复核；静态范围、Q 联动和坐标系禁用写法已取得可实现证据。Phase 5.1 控制器实测复核已确认 ROB-LTP-01..08 实测警报编号（`RBT-124/127/115/322/154-2/110/257/123`）与 CF 文档级基线对齐，详见 [LTP 专项资料包 §4.2](MACRO-LTP专项资料包.md#42-phase-51-控制器实测复核结论2026-09-22)。Rust `RobotLineState` 现有字段已覆盖 Phase 5.1 实测所需跟踪的跨行状态。对应 23 个 `SYNTEC_ROBOT_*` 稳定诊断 code（详见 [Rust诊断parity清单](../Rust诊断parity清单.md)「诊断 code ↔ 能力矩阵交叉引用」段）；原 3 个 dead code（`SYNTEC_ROBOT_SWAITSIG_Q_RANGE` / `SYNTEC_ROBOT_SYNCOUT_Q_RANGE` / `SYNTEC_ROBOT_SKIPCOND_Q_RANGE`）于 2026-09-21 Phase 5.3 路径 A 两端共同剔除，JS 总 code 67→64；原有诊断行为不变（`SYNTEC_ROBOT_STATIC_ARG_RANGE` 覆盖 SKIPCOND/SWAITSIG/SYNCOUT 三 command 的 Q range 警报）。Phase 5.1 后若需按实测 RBT 编号新增 `RBT-154-2` / `RBT-257` / `RBT-110` / `RBT-123` 等独立 code，仍按四件套登记重新落回。 | `robotValidator.js`、关键字、hover，并纳入统一 validator 流程；已覆盖 `USERCOR/TOOLCOR/G68.18/POSEMAP/SHIFTON/SKIPCOND/SWAITSIG/SYNCOUT/CIRMODE/G192.1/WAITSYNC` 等静态范围；跨行状态 `MOVC pair` / SWAITSIG/SYNCOUT counters / STITCHON/WEAVEON/WAITSYNC/G192.1 生效范围禁忌由 `RobotLineState` 状态机处理（详见 [LTP 专项资料包 §3](MACRO-LTP专项资料包.md#3-当前插件对应关系)）。 | 实测复核（CF + B 级双轨对齐） | `RBT-118` 点位偏移侧编号仍待后续专项采集；其余 RBT 编号已可用于 Phase 5.3 Rust emit 入口。 |
