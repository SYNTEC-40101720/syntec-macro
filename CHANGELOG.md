...
- **Wasm P0-B 第 2 项 完整结果— navigation parity**: Rust 试点新增 `portable_file_name` / `get_program_entry_name` / `is_macro_file_content` / `get_macro_program_name` 辅助函数（与 `src/navigationSymbols.js` 同名/语义），在 Rust 侧依据 `document.uri` 完整计算 `programEntryName`/`macroProgramName`，不再依赖 JS 适配层事后补齐；非 macro 文件输出 `navigation: null` 并清空顶层 `symbols`，镜像 JS `buildNavigationIndexEntry` 早返路径；`analyze_request` 调用 `extract_navigation` 后依据 `document.uri`/`%@MACRO` header 选择性构造 `AnalysisNavigation`，`AnalysisResult.symbols` 镜像 `AnalysisNavigation.symbols`；`compare:rust` 增加 P0-B 第 2 项 navigation 差分环节——10 项样例覆盖 macro file（含 `%.nc`/`%.cnc` 命名调用/M198）与 ISO 文件/非 macro 文件/裸 basename 边界，逐字段比较 JS 与 Rust 的 `programEntryName`/`macroProgramName`/`symbols`/`calls` (10/10 等价)。下一步进入 P0-B 编辑/TextEdit 与 profile 协商。
