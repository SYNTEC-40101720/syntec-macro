...
- **Phase 1.5 稳健 CI 采集链路就绪确认**: 审计 `.github/workflows/rust-wasm.yml` 确认双平台 perf 采集链路已经就绪不需额外 CI 改动：每次 push 到 main / PR 自动触发 ubuntu-latest + windows-latest 矩阵 job，跑 `cargo build --release --lib --target wasm32-unknown-unknown` 后做 `check:rust:wasm:asset` + `compare:rust` + `benchmark:compare --iterations 3 --no-threshold --json`，输出归档为 `perf-data/benchmark-<os>.json` 上传 `p1-perf-<os>` artifact (retention 30 天)；`cross-platform-alert` job 自动下载双平台 artifact 跑 `node scripts/comparePerfData.js` 报告 parity/fallback/regressions。user push ≥5 次后下载 5 套 artifact 用 `node scripts/comparePerfData.js perf-data/*.json` 合并查看 Rust 是否跨平台稳定，之后才走默认 backend 切换 PR + bump v3.1.0。
