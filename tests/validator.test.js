...
console.log('\n[7b] 控制流嵌套深度');
{
  const ifChain10 =
    ...
    '#1 := 1;\n' +
    'END_IF;\n'.repeat(10);
  // 关键说明：10 层正好达到上限，但 push 前判断 stack.length >= 10 才报
  // 按当前实现，第 1~10 个 IF 依次 push 时 stack.length 从 0 增至 9，均不会触发（< 10）
  eq('10 层 IF/END_IF 不触发深度 warning', ifChain10, []);

  const ifChain11 =
    'IF #1=1 THEN\n'.repeat(11) +
    '#1 := 1;\n' +
    'END_IF;\n'.repeat(11);
  // 第 11 个 IF push 前 stack.length=10，触发深度 warning（仅第 11 个）
  eq('11 层 IF 嵌套触发深度 warning',
    ifChain11,
    [['warning', 'IF 嵌套深度已达 10 层']]);
}
...
console.log('\n[9] G65/G66/G66.1 顺序');
{
  eq('G65 后仍有 G 码 → warning', '%@MACRO\nG65 P1000 G01 X10.;',
    [['warning', 'G65 必须是该行最后一个 G 码']]);
  eq('G66.1 位于最后一个 G 码 → 无警告', '%@MACRO\nG01 G66.1 P1000;', []);
  eq('注释和字符串内 G 码不参与顺序检查', '// G65 P1000 G01\nMSG("G65 P1000 G01");', []);
}
...
console.log('\n[10] 括号匹配');
{
  ...
  eq('多余右方括号', 'WHILE #203 < 10 DO]',
    [['warning', 'WHILE 块缺少对应的 END_'], ['warning', '括号不匹配：多余的右方括号']]);
  eq('间接变量右方括号配对正确', '#1 := #[10 + #2];', []);
  // 缺少右括号时，报缺少右括号
...
console.log('\n[16] 不支持的语法检测');
{
  ...
  eq('公用变量映射到 R 保留区段报 warning',
    '%@MACRO\n@481 := 1;\n@10102 := 2;\n@10513 := 3;',
    [['warning', '@481 映射到 R81'], ['warning', '@10102 映射到 R102'], ['warning', '@10513 映射到 R513']]);
  eq('公用变量映射到可写 R 区段不报',
    '%@MACRO\n@451 := 1;\n@10300 := 2;\n@16540 := 3;',
    []);
  eq('公用变量 @1~@400 不映射到 R 不报',
    '%@MACRO\n@1 := 1;\n@400 := 2;',
    []);
  eq('公用变量 @1500 非 R 映射区段不报',
    '%@MACRO\n@1500 := 1;',
    []);
  eq('公用变量 @ 大编号映射到 R0 报 warning',
    '%@MACRO\n@100000 := 1;',
    [['warning', '@100000 映射到 R0']]);
  eq('公用变量 @ 大编号 R640~R1023 区段报',
    '%@MACRO\n@101000 := 1;\n@101023 := 2;',
    [['warning', '@101000 映射到 R1000'], ['warning', '@101023 映射到 R1023']]);
  eq('公用变量 @ 大编号 R11000~R14999 区段报',
    '%@MACRO\n@111000 := 1;\n@113001 := 2;\n@114999 := 3;',
    [['warning', '@111000 映射到 R11000'], ['warning', '@113001 映射到 R13001'], ['warning', '@114999 映射到 R14999']]);
  eq('公用变量 @ R15000 可写段不报',
    '%@MACRO\n@115000 := 1;\n@165535 := 2;',
    []);
  eq('公用变量 @ R5800/R10000 可写段不报',
    '%@MACRO\n@105800 := 1;\n@110000 := 2;',
    []);
  eq('公用变量表达式索引静态不可判定不报',
    '%@MACRO\n@[#1] := 5;\n@[#1 + 400] := 10;',
    []);
...
console.log('\n[19] 机器人/坐标系旧语法诊断');
{
  ...
  eq('LTP 信号 Q 引数按来源联动检查',
    ...
    [
      ...
      ['error', 'SKIPCOND 的 Q 按 R 编号×100+bit 编码'],
      ['error', 'SWAITSIG 的 Q 引数范围为 0~511'],
      ['error', 'SWAITSIG 的 Q 按 R 编号×100+bit 编码'],
      ['error', 'SYNCOUT 的 Q 引数范围为 0~511'],
      ['error', 'SYNCOUT 的 Q 按 R 编号×100+bit 编码']
    ]);
...
console.log('\n[20] 跨行机器人/应用诊断');
{
  ...
  // L1802 静音模式版本门控（计划 β.1）
  eq('#1500=1 后跟随 G10 L1802 触发静音版本门控 warning',
    '%@MACRO\n#1500 := 1;\nG10 L1802 P500;', [['warning', '静音模式']]);
  eq('#1820=2 后跟随 G10 L1802 触发静音版本门控 warning',
    '%@MACRO\n#1820 := 2;\nG10 L1802 P300;', [['warning', '静音模式']]);
  eq('无静音赋值时 G10 L1802 不触发静音版本门控',
    '%@MACRO\nG10 L1802 P300;', []);
  eq('#1500=0 复位后 G10 L1802 不触发静音版本门控',
    '%@MACRO\n#1500 := 1;\n#1500 := 0;\nG10 L1802 P300;', []);
  eq('多次 L1802 在静音模式下各自 emit warning',
    '%@MACRO\n#1500 := 1;\nG10 L1802 P100;\nG10 L1802 P200;',
    [['warning', '静音模式'], ['warning', '静音模式']]);
}
